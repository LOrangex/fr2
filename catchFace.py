...
# 图片储存地址
PATH_FACE_SAVE = "D:\\GitHub\\fr2\\pictures\\face"
# 一个人需要取多少张训练样本和测试样本
SUM_OF_FACE_TRAIN = 200
SUM_OF_FACE_TEST = 50
# cv2人脸识别分类器地址
PATH_CLASSFIER_CV2_FRONTALFACE_ALT2 = "D:\\GitHub\\fr2\\haarcascades\\haarcascade_frontalface_alt2.xml"
# 告诉OpenCV使用人脸识别分类器
...
def discernAndSaveFace(frame, name, type, sum):
    ...
    if len(face_rects) > 0:
        for face_rect in face_rects:  # 单独框出每一张人脸
            ...
            # 统计当前捕捉的人脸数量
            cv2.putText(frame, '{} num:{}/{}'.format(type, num, sum), (l + 30, b + 30), cv2.FONT_HERSHEY_SIMPLEX, 1,
                        (255, 0, 255), 4)
...
def buildFaceDir(name):
    ...
    :param name '人名'
    '''
    dir = '{}/{}'.format('D:\\GitHub\\fr2\\pictures\\face', name)
